...
export function BlockConfigPanel({ block, stufe, onConfigChange }: Props) {
        ...
        <ConfigField label="Wortbank">
          ...
          {!wortbankAllowed && (
            <span style={{ fontSize: '0.75rem', color: 'var(--color-gray-1)' }}>
              {' '}(nur in der Unterstufe verfügbar)
            </span>
          )}
        </ConfigField>
        {wortbank && (
          <ConfigField label="Distraktoren">
            <input type="number" min={1} value={distraktoren}
              onChange={(e) => set('distraktoren', parseInt(e.target.value) || 1)} />
          </ConfigField>
        )}
      </div>
    );
  ...
  if (block.typ === 'multipleChoice') {
    const fragen = (config.fragen as Array<{ nr: number; frage: string; optionen: Array<{ key: string; text: string }>; mehrfach: boolean }>) ?? [];
    ...
    return (
      <div style={{ borderTop: '1px solid var(--color-gray-2)', paddingTop: '0.75rem', marginTop: '0.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.75rem' }}>
          <div style={{ flex: 1 }}>
            <label>Umfang min (Wörter)</label>
            <input type="number" min={1} value={umfang.min}
              onChange={(e) => set('umfangWorte', { ...umfang, min: parseInt(e.target.value) || 1 })} />
          </div>
          <div style={{ flex: 1 }}>
            <label>Umfang max (Wörter)</label>
            <input type="number" min={1} value={umfang.max}
